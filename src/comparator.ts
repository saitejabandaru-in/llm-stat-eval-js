import { mean, inverseNormalCdf } from "./stats";

export class LLMComparator {
    public nPermutations: number;
    public combinationMethod: "fisher" | "liptak" | "tippett";
    public alternative: string | string[] | Record<number, string>;
    public seed: number | null;

    // Output fields
    public partialPValues: number[] | null = null;
    public globalPValue: number | null = null;
    public observedDiffs: number[] | null = null;
    public permutationDiffs: number[][] | null = null;
    public permutationCombinedStats: number[] | null = null;
    public observedCombinedStat: number | null = null;

    constructor(
        nPermutations: number = 1000,
        combinationMethod: "fisher" | "liptak" | "tippett" = "fisher",
        alternative: string | string[] | Record<number, string> = "two-sided",
     ...
     */
    public compare(
        scoresA: number[] | number[][],
        scoresB: number[] | number[][],
        paired: boolean = true
    ): this {
        // Convert to 2D
        let dataA: number[][];
        if (Array.isArray(scoresA[0])) {
            dataA = scoresA as number[][];
        } else {
            dataA = (scoresA as number[]).map(val => [val]);
        }

        let dataB: number[][];
        if (Array.isArray(scoresB[0])) {
            dataB = scoresB as number[][];
        } else {
            dataB = (scoresB as number[]).map(val => [val]);
        }
        ...
        const obsDiffs: number[] = [];
        for (let d = 0; d < nMetrics; d++) {
            const meanA = mean(dataA.map(row => row[d]));
            const meanB = mean(dataB.map(row => row[d]));
        ...
        for (let b = 0; b < this.nPermutations; b++) {
            TPerm.push(new Array<number>(nMetrics).fill(0));
        }
        ...
        if (paired) {
            ...
            for (let d = 0; d < nMetrics; d++) {
                TPerm[0][d] = mean(diffs.map(row => row[d]));
            }
        ...
        const PPerm: number[][] = [];
        for (let b = 0; b < this.nPermutations; b++) {
            PPerm.push(new Array<number>(nMetrics).fill(0));
        }
            ...
            for (let b = 0; b < this.nPermutations; b++) {
                ...
                if (alt === "two-sided") {
                    ...
                    for (let i = 0; i < this.nPermutations; i++) {
                        if (Math.abs(statsD[i]) >= absVal) count++;
                    }
                } else if (alt === "greater") {
                    for (let i = 0; i < this.nPermutations; i++) {
                        if (statsD[i] >= val) count++;
                    }
                } else if (alt === "less") {
                    for (let i = 0; i < this.nPermutations; i++) {
                        if (statsD[i] <= val) count++;
        ...
        let globalCount = 0;
        for (let b = 0; b < this.nPermutations; b++) {
            if (CPerm[b] >= this.observedCombinedStat) globalCount++;
        }
    ...
    private _parseAlternatives(nMetrics: number): string[] {
        if (typeof this.alternative === "string") {
            ...
            return new Array<string>(nMetrics).fill(this.alternative);
            ...
            const alts = new Array<string>(nMetrics).fill("two-sided");
    ...
    private _combine(PPerm: number[][]): number[] {
        const nPerm = PPerm.length;
        const nMetrics = PPerm[0].length;
        const CPerm = new Array<number>(nPerm).fill(0);
